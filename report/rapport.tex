...
Special thanks to the summer intern Nathan Hutton. We worked on the realtime data streaming from sensors project together, and it was a great pleasure to work with him. \\
...
\begin{packed_item}
	...
	\end{packed_item}
	\item \textbf{ARMv8}: ARMv8 will introduce a 64-bit architecture support. There will also be an instruction level support for cryptography. This new architecture is in development
\end{packed_item}
...
\begin{packed_item}
	...
	\item copy the binary generated into the mbed disk
...
To establish a WebSocket connection, the client and server upgrade from the HTTP protocol to the WebSocket protocol during their initial handshake. Another attractive aspect is their HTTP-compatible handshake. This means HTTP servers can share their default HTTP (80) and HTTPS (443) ports with a WebSocket server. After the handshake, a full tcp socket communication with some overhead is available.
...
As with the realtime data streaming from sensors project, each mbed will belong to a specific subnetwork and will be identified by a specific name. This identification is made over the URL used to connect the websocket server as shown in the figure~\ref{RPC: mbed identification on a subnetwork}. \\
