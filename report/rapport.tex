...
I also want to thank two teachers of T\'{e}l\'{e}com Paristech: Mr Alexis Polti and Mr Samuel Tardieu who gave me working method and a lot of knowledge. I will never forget the two month project working with them on an octocopter. It was a unique experience. \\
...
Thus, working on the connection of different sensors to the Internet is becoming more and more crucial. I will describe in this chapter two projects concerning the Internet of Things: realtime data streaming from sensors and Remote Procedure Call mechanism. Both of these projects use a new feature of HTML5: a Webscocket communication.
...
\subsection{Why the need of Websockets ?}
...
This architecture was designed primarily for document retrieval, where web browsers load static web pages from web servers. A transaction is based on a request/response mechanism. However, web applications require more and more realtime capabilities. Developers wanted a technology where the server can send data to a client without the need of a client request. This technique is called server push. First attempts to provide realtime web applications was based on polling or server push technology:
\begin{packed_item}
	\item \textbf{polling}: the browser sends HTTP requests at regular intervals and immediately receives a response. This is a first solution but it's not optimized because if the server does not have data to send, unnecessary requests has been done and as a result there is a waste bandwidth and CPU time on both the client and the server
	\item \textbf{long polling}: a basic example is XMLHttpRequest object with long polling: The client sends requests through a XMLHttpRequest Javascript object. After each response, the client "rearms" the polling by sending a new request. The server holds client requests open until it has data to send, thus reducing unnecessary requests. But the request/response mechanism is not avoided. In high message rate situations, long polling results in a continuous loop of immediate polls. Another problem with this technique is that during the reconnection process the data on the page could be out of date and inaccurate
	\item \textbf{streaming}: The browser sends a request and the server sends and maintains an open response that is continuously updated and kept open. This is much more efficient than polling or long polling as no HTTP headers are sent during the streaming period. The network traffic is reduced. The downside of streaming is that it is still encapsulated in HTTP, so intervening HTTP proxies may choose to buffer the response, increasing the latency of the message delivery
\end{packed_item}


Ultimately, to provide full duplex communication on top of the HTTP half duplex architecture, many of today's solutions use two connections: one for the downstream and one for the upstream. The maintenance and coordination of these two connections introduces significant overhead in terms of resource consumption and adds lots of complexity. That is why a new technology called websocket has been developed. Websocket provide:
\begin{packed_item}
...
\begin{center}
\begin{lstlisting}[label=Javascript Websocket Heelo World,caption=Javascript Websocket Heelo World]
...
\begin{center}
\begin{lstlisting}[label=Manipulation of websocket messages,caption=Manipulation of websocket messages]
...
All this callback methods are called from the USB interrupt handler:
...
\chapter*{Conclusion}
By way of conclusion, I want to thank another time my manager Simon Ford. He permitted me to discover new domains involved in my projects:
\begin{packed_item}
...
Thanks to these project, I discovered web programming with in particular Javascript. I am now more familiar with web technologies such as HTML5 and websockets. It was a very good experience to develop a project which required several skills. I also really appreciated my work on the USB. It was a totally new domain for me. It was very interesting to discover the USB protocol. It was satisfying and motivating to observe days after days technical results on the USB device stack knowing that it was a totally new subject. At the end, I was very happy to provide to mbed users a USB device stack to emulate a lot of USB classes. \\


This internship was a great experience. It was a real pleasure to work in the mbed team in a very good atmosphere. Fyrthermore, carrying out this
