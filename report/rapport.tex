...
I also want to thank two teachers of T\'{e}l\'{e}com Paristech who gave me working method and a lot of knowledge. I will never forget the two month project working with them on an octocopter. It was a unique experience. \\

Special thank to the summer intern Mr Nathan Hutton. I realized the Internet of Things project with him. It was a great pleasure to work with him. \\

Finally, I would like to express my heartfelt thanks to my parents. They are always by my side. I owe them my success. 
...
More recently, a new board based on a LPC11U24 (Cortex M0) was born. This mbed has been designed to prototype USB devices or battery powered applications:
\begin{packed_item}
...
The online IDE is totally independant of the underlying operating system. It contains a lot of interesting features:
\begin{packed_item}
...
In addition to the online compiler, almost all drivers have been implemented. Users just have to instantiate an object such as SPI, I2C,... to have access to an API which abstracts all the low-level layers. \\
...
Finally, users can access the mbed forums to troubleshoot their programs or ask questions. They also have access to a handbook where there is a lot of documentation and examples concerning mbed libraries or the hardware part of the mbed. But the most important feature of this website is for me the cookbook. The cookbook is a great source of examples from other mbed users. You can almost always find a library to use with popular sensors like accelerometers, pressure sensors... The user is free to contribute to this part of the website by writing articles explaining their project and code. 
...
The figure~\ref{Reduction of polling and overhead by Websocket} shows the reduction in latency. Once the connection is established, messages can flow from the server to the browser. As the connection remains open, there is no need to send another request to the server.
...
Websocket communication involves several clients connected to the same websocket server. All messages from browsers are sent to the server. The server manages all messages. It can for instance decide to send a message received from a client to another client or to broadcast all messages received to all clients connected.
...
For example, let's say that there is an existing websocket server: example.com which is listening on port 80. A client can open a connection, receive and send messages to this server with this few lines of javascript:
...
Mbed wanted to demonstrate that it was possible to access sensor data all over the world using an mbed board and HTML5 websockets. For this, two prototype boards were developed with different sensors:
...
The idea behind this project is that the sensor data has to be available everywhere as quickly as possible. That's why we wanted to involve smartphones connected to the Internet over 3G. To achieve this, we used QR codes to speed up the whole process. The steps to access data for a user are:

\begin{packed_item}
	\item Place the objects (mbed and sensors) where desired
...
All mbed boards send messages in JSON format to the websocket server containing sensor data. The websocket server will broadcast to all client connected all messages received, so all browsers connected to this server are able to receive data from sensors.
...
The websocket server which has been developed uses Tornado framework. This framework is written in Python, and it's a non blocking web server. It uses epoll which permits the handling of thousands of connections and so provides realtime features. Some interesting features are, for instance:
\begin{packed_item}
	\item http server
...
\begin{packed_item}
	\item smartphone webpages: display only the graph of the corresponding QR code scanned. Two buttons can be used to add or remove a specific graph of the dashboard webpage
	\item dashboard: display all graphs added by users
\end{packed_item}

\subsubsection{Realtime graphs}
I use Smoothie Charts which is a javascript library to stream data on a canvas element. Smoothie Charts is a small charting library designed for live streaming data. The idea is to display in realtime sensor data in a chart refreshed everytime that we receive a new data.
...
Messages sent by mbed boards contain sensor data. When a client receives such a message, it refreshes a realtime graph showing the sensor data against time. \\
...
After the Internet of Things project demonstrated the power of HTML5 in embedded systems, mbed wanted to use websockets to design a RPC (Remote Procedure Call) mechanism. The idea is to enable users to call a specific method on a specific mbed. Both of these mbeds are connected to the Internet.
...
\section{Conclusion}
For Mbed the Internet of Things is a very important field. We really think that in the future more and more devices will be connected to the cloud. Examples of applications could include:
\begin{packed_item}
...
\begin{packed_item}
	...
	\item \textbf{USB class layer}:
		\begin{packed_item}
			\item USBHID: implements standard requests of the HID class specification. When a USBHID object is instantiated, the mbed is enumerated as a generic HID device so that raw data can be sent and receive to and from a custom program running on the host side
...
\subsubsection{Audio packets management}
...
The main part of sending and reception of audio packets is in the virtual function called in USBHAL on each Start Of Frame event:
...
\section{Conclusion}
Mbed users can now use their mbed to emulate a USB device. I really wanted explore all capabilities of USB. That is why several classes has been developed:
\begin{packed_item}
	\item Human Interface Device (HID) which is in my opinion the best way to develop a custom USB device because it permits to exchange raw data without the need of a specific driver on the host side.
...
\chapter{Conclusion}
By way of concluion, I want to thank another time my manager Simon Ford. He permitted me to discover new domains involved in my projects:
\begin{packed_item}
...
The Internet of Things and the RPC projects was very interesting because it needed several skills such as:
\begin{packed_item}
	\item software skills in embedded systems to program the mbed
	\item software skills in web programming
	\item hardware skills
\end{packed_item}
Thanks to these project, I discovered web programming with in particular Javascript. I am now more familiar with web technologies such as HTML5 and websockets. It was a very good experience to develop a project which required several skills. \\

I really appreciated my work on the USB. It was a totally new domain for me. It was very interesting to discover the USB protocol. It was satisfying and motivating to observe days after days technical results on the USB device stack knowing that it was a totally new subject. At the end, I was very happy to provide to mbed users a USB device stack to emulate a lot of USB classes. \\


This internship was a great experience. It was a real pleasure to work in the mbed team in a very good atmosphere. Carrying out this
internship in a foreign country allows me to be immersed in a different culture and faced the problem of living and working entirely in a non-native language- English. I met several
interesting people in and out of the lab such as engineers, students, but also people not connected with computer science.
...
\begin{comment}
My project during this internship was to  design the drawings to manufacture the box 
connecting the cryostat and the detector by the Berkeley labs workshop. I started by defining
what we needed to accomplish  that. Then we designed all the drawings and ordered all the 
components we wante d for our box. We finished by  choosing and buying the inside optical
equipment. However, the project isn’t finished and someone must continue it to arrive at a 
useful application for the beamline.
Concerning this project, it was interesting to discover some thing else that I was taught in 
my engineering school such as electronics or spectroscopy in x-ray region. The work in itself 
was also interesting in different point of views.  Designing the box drawings allowed me to 
learn some new design techniques.
Moreover, working on a concrete problem made me learn several things. Even though that 
was something new for me, I could use tools I knew to find solutions like cutting a big
parabolic mirror to make several little ones with the focal distance we need. Now, I
understand the principle of a cryostat thanks to what I learnt 2 or 3 years ago in
thermodynamic and the process for manufacturing equipment such as this. I  was  already 
aware of much of this,  but here I could see for real that besides the tools we learn in our 
studies, what is really useful is the scientific approach we acquired to solve problems. Then, 
talking with people and exchanging ideas, I came up to the view factor. 
This internship was a great experience. It was a real pleasure to work in this ALS at the 
beamline 1.4. I could work with nice people in a good atmosphere. Carrying out this
internship in a foreign country allows me to be immersed in a different culture and faced the 
problem of living and working entirely in a non-native language- English. I met several
interesting people in and out of the lab such as researchers, students in physics, but also 
people not connected with science
\end{comment}
...
\begin{lstlisting}[label=USBHAL::usbisr(),caption=USBHAL::usbisr()]
