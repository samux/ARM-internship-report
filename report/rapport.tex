...
\begin{packed_item}
	\item First I will explain how to use microcontrollers to prototype an Internet Of Things projects where will be connected to the Internet. The objective of this project is to access sensor data all over the world introducing a new HTML5 feature named WebSocket
...
I really wanted to test a WebSocket communication with an mbed allowing it to be a client. With this library, an mbed is able to send and receive messages to a WebSocket server. To achieve this I just need a TCP socket:
\begin{packed_item}
	\item After having opened the socket, HTTP headers for the WebSocket handshake are sent to the server
	\item If the server accepts the handshake, it sends back the response confirming the upgrade to the WebSocket protocol 
...
Eventually, the mbed library abstracts the type of connection and can be used either with an Ethernet or a wireless connection. For instance, the method which connects an mbed to a WebSocket server over wifi is:
...
For this library, I wanted to provide a simple but consistent API for mbed users. This API is as follows:
...
A USB device stack has been developed to allow mbed users to design their own USB device or to use their mbed to emulate a USB peripheral such as a keyboard or a mouse. I did not implemented the USB device stack from scratch but I had sources of a previous project. I reused these C low level layers of this project and adapted them in a more C++ structured architecture. I have added new functionnalities to these low layers to support for instance all kinds of endpoints and implemented several USB classes:
\begin{packed_item}
...
In my opinion, the most important mechanism that need to be mentionned concerns all virtual methods called from USBHAL. When a specific event occurs, USBHAL calls specific methods which are virtuals. They can then be overriden in a subclass to perform a custom treatment. Main callback functions are:
\begin{packed_item}
...
I decided to use virtual methods because it's very easy for a user to inherit from this class and define his own treatment on specific events. 
...
The following code contains an example of configuration descriptor for a USB HID device. As descriptors can be very long, I only provide this example. I will not explicit all other USB descriptors. But they can be easily found in the specification document of a USB class.
...
In my opinion, HID is the best class to design a custom USB device. I mean that it is possible to send and receive raw data packets to and from the host without any protocol. But the most important thing is that no driver is needed on the host side as all operating systems have a built-in HID driver. I really wanted to provide to mbed users a way to exchange raw data with a custom program in Python, C/C++, Java,... They can create a generic HID device by instantiating a USBHID object. \\

As all HID devices, a report descriptor as to be provided in order to describe data exchanged. To summarize the descriptor, the device can send and receive byte packets of a certain length. Additionnally, this class defines methods to send and receive reports. Data are sent or read by writing or reading the endpoints specified in the confiiguration descriptor:
...
TODO: put screenshot of windows device recognized and python detecting the device
...
\begin{packed_item}
	\item Mouse buttons: There are 3 buttons represented by 1 bit. Values for these buttons are 0 or 1 (pressed or not).
	\item X, Y and scroll: 3 different features, each represented by 1 byte. Values can vary between -127 and 127.
\end{packed_item}
...
Once the device is enumerated, data can be exchanged over the two bulk endpoints. To send a character, we just write the character on the bulk IN endpoint. Characters received are pushed on a circular buffer. This step is done in the EPx\_OUT\_callback() virtual function called when a packet is received. We can then define two methods to send and receive data:
